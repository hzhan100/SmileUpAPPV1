package com.smileup.customviews;
...
public class RoundedImageView extends ImageView {
	...
	private WeakReference<Bitmap> roundImageReference;
	private WeakReference<Bitmap> copiedImageReference;
	...
	protected void onDraw(Canvas canvas) {
		...
		copiedImageReference = new WeakReference<Bitmap>(bitmap);

		int w = getWidth(), h = getHeight();

		int radius = (w < h ? w : h) / 2 - mBorderThickness;
		roundImageReference = new WeakReference<Bitmap>(getCroppedBitmap(
				copiedImageReference, radius));
		...
		canvas.drawBitmap(roundImageReference.get(), w / 2 - radius, h / 2
				- radius, null);

	}

	public static Bitmap getCroppedBitmap(WeakReference<Bitmap> bmpRef,
			int radius) {
		Bitmap scaledSrcBmp;
